...
def _norm(v) -> str:
    """Normalize values for reliable matching and handle NaN"""
...
def _map_capacity(cap_val, is_publisher=False):
    ...
    if not cap_val or pd.isna(cap_val):
        return ""
...
def process_alternate_titles(curve_reexport_buffer, jotform_file_buffer) -> io.BytesIO | str:
    try:
        ...
        if ws_alt.max_row > 2: ws_alt.delete_rows(3, ws_alt.max_row - 2)
        if ws_ip.max_row > 2: ws_ip.delete_rows(3, ws_ip.max_row - 2)
        ...
        for r in range(2, ws_works.max_row + 1):
            ...
            if matched_jot_row is not None:
                ...
                if jot_writer_total_col:
                    try:
                        val = matched_jot_row.get(jot_writer_total_col, 0)
                        writer_total = int(float(val)) if not pd.isna(val) else 0
                    except:
                        writer_total = 0
                ...
                for i in range(1, min(writer_total + 1, 11)):
                    ...
                    c_controlled_col = _find_jot_col(jot, [[f"COMPOSER {i} CONTROLLED"]])
                ...
                rows_to_create = []
                if controlled_group: rows_to_create.append({"type": "controlled", "writers": controlled_group})
                if other_group: rows_to_create.append({"type": "other", "writers": other_group})
